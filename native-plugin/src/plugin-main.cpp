...
void load_overlay_position() {
  ...
  blog(LOG_INFO, "[obs-overlay-time-show] arquivo de posicao: %s",
...
obs_hotkey_id register_one_hotkey(const char *id, const char *lookup_key,
  ...
  if (hotkey_id == OBS_INVALID_HOTKEY_ID) {
    ...
    blog(LOG_INFO, "[obs-overlay-time-show] atalho registrado: %s (%s)", id,
         description);
  }
...
void save_hotkey_array(obs_data_t *save_data, const char *key,
                       obs_hotkey_id id) {
  if (id == OBS_INVALID_HOTKEY_ID) {
    return;
  }
...
void load_hotkey_array(obs_data_t *save_data, const char *key,
                       obs_hotkey_id id) {
  if (id == OBS_INVALID_HOTKEY_ID) {
    return;
  }
